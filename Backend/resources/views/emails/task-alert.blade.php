...
<head>
    ...
    <title>Alerta de Tarea - {{ $task->title }}</title>
    <style>
        * {
            margin: 0;
            padding: 0;
        ...
        .header {
            background: {{ $alert->type === 'Critical' ? 'linear-gradient(135deg, #dc2626 0%, #991b1b 100%)' : 'linear-gradient(135deg, #f59e0b 0%, #d97706 100%)' }};
            color: #ffffff;
            padding: 45px 35px;
            text-align: center;
        }
        ...
        .alert-box {
            background-color: {{ $alert->type === 'Critical' ? '#fef2f2' : '#fffbeb' }};
            border-left: 4px solid {{ $alert->type === 'Critical' ? '#dc2626' : '#f59e0b' }};
            padding: 20px;
            margin: 28px 0;
            border-radius: 6px;
        }
        ...
        .alert-box p {
            margin: 8px 0;
            font-size: 15px;
            color: {{ $alert->type === 'Critical' ? '#7f1d1d' : '#78350f' }};
            line-height: 1.6;
        }
        .alert-box strong {
            color: {{ $alert->type === 'Critical' ? '#991b1b' : '#92400e' }};
...
<body>
    <div class="email-wrapper">
        <div class="container">
            ...
            <div class="content">
                <p class="greeting">Estimado/a {{ $user->name }},</p>
                
                <div class="alert-box">
                    <h3>{{ $alert->message }}</h3>
                    <p><strong>Tipo de Alerta:</strong> {{ $alert->type }}</p>
                    <p><strong>Fecha:</strong> {{ $alert->created_at->format('d/m/Y H:i') }}</p>
                </div>
                ...
                <div class="details-box">
                    ...
                    <div class="detail-row">
                        ...
                        <div class="detail-value">
                            <span class="risk-badge risk-{{ strtolower($task->risk_level) }}">
                                {{ $task->risk_level }}
                            </span>
                        </div>
                    </div>
                ...
                <div class="button-container">
                    ...
                    <a href="{{ config('app.frontend_url') }}/tasks/alerts" class="btn">
                        Ver Alertas
                    </a>
                </div>
                ...
                @if($alert->type === 'Critical')
                    <div class="warning-box">
                        <p><strong>Acción Requerida:</strong> Esta es una alerta crítica que requiere atención inmediata. 
                        Por favor, revise la tarea y tome las medidas necesarias.</p>
                    </div>
                @endif
            </div>
