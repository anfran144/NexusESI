...
export const Route = createFileRoute('/_authenticated/coordinator/eventos/$eventId/alerts')({
  component: EventAlertsPage,
})
...
function EventAlertsPage() {
  ...
  if (loading) {
    return (
      <DashboardLayout
        title="Cargando..."
        description="Alertas del evento"
        showFooter={false}
      >
        <DashboardContent>
  ...
  return (
    <PermissionGuard permission="events.alerts.view">
      <DashboardLayout
        title="Mis Alertas"
        description={`Alertas y notificaciones de ${event.name}`}
        showFooter={true}
      >
        <DashboardContent>
          <div className="space-y-6">
            ...
            <AlertList eventId={Number(eventId)} showFilters={true} />
          </div>
        </DashboardContent>
      </DashboardLayout>
    </PermissionGuard>
